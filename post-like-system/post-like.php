...
function sl_enqueue_scripts() {
	wp_enqueue_script( 'simple-likes-public-js', plugin_dir_url( __FILE__ ) . 'js/simple-likes-public-min.js', array( 'jquery' ), '0.5', false );
	wp_localize_script( 'simple-likes-public-js', 'simpleLikes', array(
		'ajaxurl' => admin_url( 'admin-ajax.php' ),
		'like' => esc_html__( 'Like', 'alc-advanced-posts' ),
		'unlike' => esc_html__( 'Unlike', 'alc-advanced-posts' )
	) );
}
...
function process_simple_like() {
	...
	if ( !wp_verify_nonce( $nonce, 'simple-likes-nonce' ) ) {
		exit( esc_html__( 'Not permitted', 'alc-advanced-posts' ) );
	}
...
function get_simple_likes_button( $post_id, $is_comment = NULL ) {
	...
	if ( already_liked( $post_id, $is_comment ) ) {
		...
		$title = esc_html__( 'Unlike', 'alc-advanced-posts' );
		$icon = $icon_full;
	} else {
		$class = '';
		$title = esc_html__( 'Like', 'alc-advanced-posts' );
		$icon = $icon_empty;
	}

	$output = '<div class="meta__item meta__item--likes ' . $class . '"><a href="' . admin_url( 'admin-ajax.php?action=process_simple_like' . '&post_id=' . $post_id . '&nonce=' . $nonce . '&is_comment=' . $is_comment . '&disabled=true' ) . '" class="sl-button' . $post_id_class . $comment_class . '" data-nonce="' . $nonce . '" data-post-id="' . $post_id . '" data-iscomment="' . $is_comment . '" title="' . $title . '">' . $icon . $count . '</a>' . $loader . '</div>';
	return $output;
...
function get_liked_icon() {
	$icon = '<span class="sl-icon"><i class="meta-like icon-heart"></i></span>';
	return $icon;
} // get_liked_icon()
